...
def main(file_path):
    ...
    loader = file_loader.ParallelLoader(file_path, lib.CHUNK_SIZE, BYTES_PER_STEP)

    # create progressbars
    LOGGER(f"File size: {loader.file_size:,.0f} B")
    training_bar = tqdm(total=EPOCHS, unit='Epochs', position=1)
    epoch_bar = tqdm(total=loader.file_size, unit='B', unit_divisor=1024, unit_scale=True, position=0, leave=True)
...
def evaluate(model: torch.nn.Module, loader: file_loader.ParallelLoader):
    model.eval()
    ...
    with torch.no_grad():
        ...
        # keep track of standard deviation of model predictions
        num_batches = 0
        avg_std = 0.
